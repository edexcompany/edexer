...
public class UserServiceManagerImpl implements UserServiceManager {
	...
	public Integer insertUser(User user) {
		...
		try {
			...
			r.setRoleId(2);
			user.setRole(r);
			userId = (int) userDao.add(user);
			UserStatus status = new UserStatus();
			status.setUserStatusId(Integer.valueOf(settings
					.getString("USER_STATUS_ACTIVE")));
			user.setUserStatus(status);
			userSubscriptionService.insertCorpUserSubscription(userId,
					Constants.CORP_OWNER, Constants.CORP_OWNER);
		} catch (Exception e) {
			e.printStackTrace();
			return 0;
		}

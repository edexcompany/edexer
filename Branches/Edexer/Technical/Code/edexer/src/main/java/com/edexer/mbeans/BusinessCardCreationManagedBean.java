...
public class BusinessCardCreationManagedBean implements Serializable {
	...
	/*
	 * @ManagedProperty("userSettingsManagedBean") private
	 * UserSettingsManagedBean userSettingsManagedBean;
	 */
	...
	public void actionSaveforavatar() {
		logger.info("Action Save for avatar Called");
		// prepareBusinessCard();
		try {
			if (file != null) {
				user.setUserAvatar(file.getFileName());
			}
			FilesUtil.uploadFile(file, user.getUserId());
			System.out.println("image saved in data base !! ");
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private void prepareBusinessCard() {
		...
		UserSubscription parentCorpSub = new UserSubscription();
		parentCorpSub.setId(new UserSubscriptionId(Constants.CORP_OWNER,
				Constants.CORP_SUB_TYPE));
		...
		businessCard.getBcPermissionses().add(
				new BcPermissions( (UserSubscription) user
						.getUserSubscriptionsForUserId().iterator().next(),businessCard));
		UserSubscription us;
	...
	/*
	 * public UserSettingsManagedBean getUserSettingsManagedBean() { return
	 * userSettingsManagedBean; }
	 * 
	 * public void setUserSettingsManagedBean( UserSettingsManagedBean
	 * userSettingsManagedBean) { this.userSettingsManagedBean =
	 * userSettingsManagedBean; }
	 */

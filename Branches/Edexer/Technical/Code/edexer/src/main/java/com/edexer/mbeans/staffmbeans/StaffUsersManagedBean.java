...
public class StaffUsersManagedBean implements Serializable {
	...
	public String addUser() {
		...
		try {
			...
			if (!userService.isUserExists(userEmail)) {
				...
				error = 1;
				sendEmail(rawPassword);
				FacesUtil.addInfoMessage("User Added", "");
				return Constants.INDEX;
			} else {
				FacesUtil.addErrorMessage("Error. User exists.",
	...
	private void sendEmail(String rawPassword) throws Exception {
		String to = newUser.getUserEmail();
		String title = "FindOn account";
		
		String body = "Dear " + newUser.getFirstName()
				+ "You have new account on edexer.com , your password: "
				+ rawPassword;
		String fromEmail = settings.getString("WEBMASTER_EMAIL");
		mailService.sendMail(fromEmail, newUser.getUserEmail(), title, body);
	}

...
public class BusinessCardServiceManagerImpl implements
	...
	public boolean checkCardViewAuthority(User user, BusinessCard businessCard) {
		UserSubscription us = (UserSubscription) user
				.getUserSubscriptionsForUserId().iterator().next();
		if (us.getIsAdmin().equals("Y"))
			return true;
		UserSubscription parentCorpUserSubscription = userSubscriptionService
				.getParentCorpSubscriptionByUser(user);
		if (/* case corp */parentCorpUserSubscription != null
			...
			return checkViewAuthority(user, businessCard);
		return false;
	}
	...
	public String validateBusinessCard(BusinessCard bc, boolean updateMode) {
		...
		while (it.hasNext()) {
			...
			System.out.println("regex:"
					+ bcValidations.getString("PHONE_REGEX"));
			System.out.println(num.matches(bcValidations
					.getString("PHONE_REGEX")));
		...
		if (duplicationError != null) {
			...
			System.out.println(errorMsg + "so update can not be done!!!");
	...
	private boolean checkExportAuthority(User user, BusinessCard bc) {
		return true;
		// if (getBusinessCardType(bc) == BusinessCardTypes.Corporate) {
	...
	public boolean checkViewAuthority(User user, BusinessCard bc) {

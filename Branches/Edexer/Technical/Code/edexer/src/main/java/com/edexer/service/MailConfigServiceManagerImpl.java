...
public class MailConfigServiceManagerImpl implements MailConfigServiceManager {
	...
	public String InsertOrUpdateMailConfig(MailConfig mail, User user) {
		try {
			...
			if (obj == null) {
				//mail.setFromEmailPassword(enClass.encrypt(mail.getFromEmailPassword()));
				mailConfigDao.add(mail);
			} else if (obj != null) {
				obj.setUser(mail.getUser());
				obj.setFromEmail(mail.getFromEmail());
				//obj.setFromEmailPassword(enClass.encrypt(mail.getFromEmailPassword()));
	...
	public String sendEmail(User user, String subject, String body,
			List<BusinessCard> bCard) throws Exception {
		try {
			...
			mConfig.setFromEmailPassword(enClass.encrypt(mConfig.getFromEmailPassword()));
			return null;
		} catch (Exception ex) {
			ex.printStackTrace();
			throw ex;

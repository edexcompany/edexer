...
public class BusinessCardsIndexManagedBean implements Serializable {
	...
	public void init() {
		...
		int corpSubscriptionOwnerId = (corpSubscription == null ? 0
...
public Boolean retIsProtectedOrNot(Integer userid) {
		...
		try {
			...
			if (userid != null) {
				...
				if (user != null) {
					if (user.getPrivacyLevel().getPrivacyName()
							.equals("Protected") || user.getPrivacyLevel().getPrivacyName()
							.equals("Public") ) {
						User senderUser = HttpSessionUtil.getUser();
						List<BcRequest> bcRequest = new ArrayList<BcRequest>();
						bcRequest = bcRequestservice
								.listBcRequestsBySenderANDreciver(senderUser,
										user);
						if (bcRequest != null && bcRequest.size() > 0) {
							for (BcRequest request : bcRequest) {
								if (request.getBcRequestStatus()
										.getStatusName().equals("Accept")) {
									return protect;
								} else {
									protect = true;
								}
	...
	public Boolean retIsPublicOrNot(Integer userid) {
		Boolean pub = false;
		try {
			User user = new User();
			if (userid != null) {
				...
				if (user != null) {
					if (user.getPrivacyLevel().getPrivacyName()
							.equals("Public")) {
						pub = true;
					} else if (user.getPrivacyLevel().getPrivacyName()
							.equals("Protected")) {
						User senderUser = HttpSessionUtil.getUser();
						List<BcRequest> bcRequest = new ArrayList<BcRequest>();
						bcRequest = bcRequestservice

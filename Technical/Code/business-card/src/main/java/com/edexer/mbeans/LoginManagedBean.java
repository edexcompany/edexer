...
public class LoginManagedBean implements Serializable {
	...
	/*@PostConstruct
	void init() {
		reciverEmail = HttpSessionUtil.getSession().getAttribute("reciver")
				.toString();
	...
	public String loginAction() {
		...
		try {
			...
			if (userRole != null
					&& userRole.getRoleId() == Integer.valueOf(settings
							.getString("ROLE_ADMIN_ID"))) {
				logger.debug("role is ROLE_ADMIN_NAME: "
						+ settings.getString("ROLE_ADMIN_NAME"));
				return settings.getString("ROLE_ADMIN_NAME");
			} else if (userRole != null
					&& userRole.getRoleId() == Integer.valueOf(settings
							.getString("ROLE_USER_ID"))) {
				logger.debug("role is ROLE_USER_NAME: "
						+ settings.getString("ROLE_USER_NAME"));
				return settings.getString("ROLE_USER_NAME");
			} else {
				logger.debug("There is no role assigned to your user");
				FacesUtil
						.addErrorMessage(Constants.LOGIN_ERROR,
								"There is no role assigned to your user, please contact Customer support!");
				return Constants.INDEX;
			}

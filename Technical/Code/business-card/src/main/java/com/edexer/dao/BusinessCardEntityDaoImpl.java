...
public class BusinessCardEntityDaoImpl extends
	...
	public String validateBusinessCard(BusinessCard businessCard,
		...
		if (!businessCard.getEmails().isEmpty() && error.equals("")) {
			...
			while (it.hasNext()) {
				if (emailDao.subscriptionHasEmail(((Email) it.next()).getId()
						.getEmailAddress(), businessCard.getUserSubscription(),
						(updateMode ? businessCard.getBusinessCardId() : 0))) {
					error = "e";
					break;
				}
	...
	public void update(BusinessCard entity, UploadedFile avatar,
		...
		while (it.hasNext()) {
			...
			// logger.debug("adding title sector id" +
			// t.getSector().getSectorId());
			titleDao.add(t);
		}
		it = entity.getWebsites().iterator();
		while (it.hasNext()) {
	...
	public BusinessCard getFinOnCardByCreator(Integer userId) {
		BusinessCard card = new BusinessCard();
		Session session = sessionFactory.getCurrentSession();
		ArrayList<BusinessCard> list = null;
		Query q = session
				.createQuery("from BusinessCard where creator=:userid and subscription_sub_type IS NULL and subscription_user_id IS NULL ");
		q.setParameter("userid", userId);
		list = (ArrayList<BusinessCard>) q.list();
		// logger.info("Returning from getAdminNotes");
		if (list != null && list.size() > 0) {
			card = (BusinessCard) list.get(0);
			card=get(card.getBusinessCardId(), true);
		}
		return card;
	}
}

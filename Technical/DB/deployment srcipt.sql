...
-- Schema edexer
-- -----------------------------------------------------
DROP SCHEMA IF EXISTS `edexer`;
CREATE SCHEMA IF NOT EXISTS `edexer` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci ;
USE `edexer` ;

-- -----------------------------------------------------
-- Table `edexer`.`user`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`user` (
...
-- Table `edexer`.`role`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`role` (
...
-- -----------------------------------------------------
-- Table `edexer`.`subscription`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`subscription` (
...
-- -----------------------------------------------------
-- Table `edexer`.`subscription_status`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`subscription_status` (
...
-- Table `edexer`.`act_as`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`act_as` (
...
-- -----------------------------------------------------
-- Table `edexer`.`last_edit_reason`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`last_edit_reason` (
...
-- -----------------------------------------------------
-- Table `edexer`.`user_subscription`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`user_subscription` (
  ...
  CONSTRAINT `fk_user_subscription_subscription1`
    ...
    REFERENCES `edexer`.`subscription` (`subscription_id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  ...
  CONSTRAINT `fk_user_subscription_subscription_status1`
    ...
    REFERENCES `edexer`.`subscription_status` (`sub_status_id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_user_subscription_act_as1`
    FOREIGN KEY (`act_as`)
    REFERENCES `edexer`.`act_as` (`act_as_id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_user_subscription_last_edit_reason1`
    FOREIGN KEY (`last_edit_reason`)
    REFERENCES `edexer`.`last_edit_reason` (`reason_id`)
    ON DELETE SET NULL
    ON UPDATE RESTRICT)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `edexer`.`business_card`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`business_card` (
  `business_card_id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `bc_first_name` VARCHAR(100) NULL,
  `middle_name` VARCHAR(100) NULL,
  `bc_last_name` VARCHAR(100) NULL,
...
-- -----------------------------------------------------
-- Table `edexer`.`user_role`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`user_role` (
  ...
  CONSTRAINT `fk_User_has_role_role1`
    ...
    REFERENCES `edexer`.`role` (`role_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- Table `edexer`.`tags`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`tags` (
  ...
  CONSTRAINT `fk_tags_user_subscription1`
    ...
    REFERENCES `edexer`.`user_subscription` (`user_id` , `sub_type`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `edexer`.`attachment`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`attachment` (
  ...
  CONSTRAINT `fk_attachment_business_card1`
    ...
    REFERENCES `edexer`.`business_card` (`business_card_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
-- Table `edexer`.`mobile`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`mobile` (
...
-- -----------------------------------------------------
-- Table `edexer`.`website`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`website` (
  `bc_id` INT UNSIGNED NOT NULL,
...
-- Table `edexer`.`phone`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`phone` (
  `bc_id` INT UNSIGNED NOT NULL,
...
-- Table `edexer`.`fax`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`fax` (
  `bc_id` INT UNSIGNED NOT NULL,
...
-- Table `edexer`.`email`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`email` (
  `bc_id` INT UNSIGNED NOT NULL,
...
-- Table `edexer`.`im`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`im` (
  `bc_id` INT UNSIGNED NOT NULL,
...
-- -----------------------------------------------------
-- Table `edexer`.`address`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`address` (
  `business_card_business_card_id` INT UNSIGNED NOT NULL,
...
-- -----------------------------------------------------
-- Table `edexer`.`social_networks_types`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`social_networks_types` (
...
-- -----------------------------------------------------
-- Table `edexer`.`social_network`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`social_network` (
  `bc_id` INT UNSIGNED NOT NULL,
  `identifier` VARCHAR(50) NOT NULL,
  ...
  CONSTRAINT `fk_social_network_social_networks_types1`
    FOREIGN KEY (`type`)
    REFERENCES `edexer`.`social_networks_types` (`sn_type_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
-- Table `edexer`.`sector`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`sector` (
...
-- Table `edexer`.`title`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`title` (
  ...
  CONSTRAINT `fk_title_sector1`
    ...
    REFERENCES `edexer`.`sector` (`sector_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
...
-- -----------------------------------------------------
-- Table `edexer`.`bc_tags`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`bc_tags` (
  `bc_id` INT UNSIGNED NOT NULL,
...
-- -----------------------------------------------------
-- Table `edexer`.`bc_permissions`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`bc_permissions` (
...
#--------------------------------------------------------
#-- Table `edexer`.`admin_config`
#--------------------------------------------------------
CREATE  TABLE `edexer`.`admin_config` (
  `config_key_name` VARCHAR(100) NULL ,
  `config_value` VARCHAR(150) NULL );

#--------------------------------------------------------
ALTER TABLE `edexer`.`user` ADD COLUMN `user_role` INT UNSIGNED NULL  AFTER `title` ;
ALTER TABLE `edexer`.`user` ADD COLUMN `activation_str` VARCHAR(200) NULL  AFTER `user_role` ;
#-------------------------------------------------------
ALTER TABLE `edexer`.`user` 
...
CREATE TABLE IF NOT EXISTS `edexer`.`countries` (
...
ADD CONSTRAINT `fk_address_country`
  FOREIGN KEY (`country`)
  REFERENCES `edexer`.`countries` (`idCountry`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;
...
ALTER TABLE `edexer`.`business_card` 
...
alter table edexer.business_card add  
...
-- TEST DATA
INSERT INTO `edexer`.`role` (`role_id`, `role_name`, `role_desc`) VALUES ('1', 'Admin', 'Staff admin');
INSERT INTO `edexer`.`role` (`role_id`, `role_name`, `role_desc`) VALUES ('2', 'User', 'Normal User, can be either user or subc. admin');


INSERT INTO `edexer`.`subscription` (`subscription_id`, `subscription_type_name`, `desc`) VALUES ('1', 'Personal-Free', 'Free subscription');
INSERT INTO `edexer`.`subscription` (`subscription_id`, `subscription_type_name`, `desc`) VALUES ('2', 'Personal-Pro', 'Personal Pro subscription for a monthly fees');
INSERT INTO `edexer`.`subscription` (`subscription_id`, `subscription_type_name`, `desc`) VALUES ('3', 'Coporate', 'a Corp subscription for a monthly fees');

INSERT INTO `act_as` VALUES (1,'Admin','1'),(2,'Normal User','users other than admin'),(3,'Owner',NULL);

ALTER TABLE `edexer`.`attachment` 
DROP PRIMARY KEY,
...
ALTER TABLE `edexer`.`user_subscription` ADD COLUMN `admin_user` INT UNSIGNED NULL  AFTER `note` ;
ALTER TABLE `edexer`.`user_subscription` 
 ...
 ALTER TABLE `edexer`.`user_subscription` ADD COLUMN `subscription_owner` INT(10) UNSIGNED NULL  AFTER `admin_user` , 
...
ADD CONSTRAINT `fk_user_subscription_subscription1`
  ...
  ALTER TABLE `edexer`.`bc_permissions` 
...
CREATE TABLE `edexer`.`user_status` (
...
ADD CONSTRAINT `FK_status_id`
  FOREIGN KEY (`status`)
  REFERENCES `edexer`.`user_status` (`user_status_id`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;

INSERT INTO `edexer`.`user_status` (`status`) VALUES ('ACTIVE');
INSERT INTO `edexer`.`user_status` (`status`) VALUES ('DELETED');
INSERT INTO `edexer`.`user_status` (`status`) VALUES ('SUSPENDED');
INSERT INTO `edexer`.`user_status` (`status`) VALUES ('BLOCKED');
...
CREATE TABLE `edexer`.`password_reset` (
...
ALTER TABLE `edexer`.`password_reset` 
ADD COLUMN `used` VARCHAR(45) NULL DEFAULT 'N' AFTER `user_id`;

ALTER TABLE `edexer`.`user` 
ADD COLUMN `salt` VARCHAR(50) NULL AFTER `password`;

INSERT INTO `edexer`.`user` (`user_email`, `first_name`, `last_name`, `password`, `salt`, `status`, `roleid`) VALUES ('admin@findon.com', 'Web', 'Master', '07f423331cfbf492819d7f15ee33880af2b75e9675b6d07d', '83aca74aa19ed370a71f9dbff86d054724fae9ad87d60547', '1', '1');
...
-- -----------------------------------------------------
-- Table `edexer`.`privacy_level`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`privacy_level` (
...
ADD CONSTRAINT `fk_user_privacy`
  ...
  REFERENCES `edexer`.`privacy_level` (`privacy_id`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;
-- -----------------------------------------------------
-- Table `edexer`.`bc_request_status`-------------------
-- ------------------------------------------------------
CREATE TABLE IF NOT EXISTS `edexer`.`bc_request_status` (
...
-- ------------------------------------------------------
INSERT INTO `edexer`.`bc_request_status` (`status_name`, `desc`) VALUES ('Accept', 'accept user request');
INSERT INTO `edexer`.`bc_request_status` (`status_name`, `desc`) VALUES ('Ignore', 'refuse user request');
INSERT INTO `edexer`.`bc_request_status` (`status_name`, `desc`) VALUES ('Sent', 'request had sent to receiver');
-- -Insert Privacy Level Types--------------------------
-- ------------------------------------------------------
INSERT INTO `edexer`.`privacy_level` (`privacy_name`, `desc`) VALUES ('Public', 'show all personal data for any user');
INSERT INTO `edexer`.`privacy_level` (`privacy_name`, `desc`) VALUES ('Private', 'hide all personal data from any user');
INSERT INTO `edexer`.`privacy_level` (`privacy_name`) VALUES ('Protected');

-- -create new table for set home messages-------------
-- -----------------------------------------------------
...
FROM
    ...
    edexer.user_subscription AS us ON bc.subscription_sub_type = us.sub_type
        ...
        LEFT JOIN 
	edexer.user As user ON bc.subscription_user_id=user.user_id
		Left JOIN 
    edexer.privacy_level As privacy ON user.privacy_level_id=privacy.privacy_id
       LEFT JOIN
...
WHERE 
   (bc.creator = ANY (select u.user_id from user u INNER JOIN privacy_level p
						ON u.privacy_level_id = p.privacy_id 
						where p.privacy_name  !='Private' or (bc.subscription_sub_type IS NOT NULL and bc.subscription_user_id IS NOT NULL)))
 AND   (CASE
    #subscriptions part
        WHEN
            sub_type_param = 1
